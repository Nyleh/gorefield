...
function onNoteCreation(event) {
	...
	if (event.note.isSustainNote) {
		note.loadGraphic(Paths.image('game/pixel/Notes_Assets_End'), true, 7, 6);
		note.animation.add("hold", [event.strumID]);
		note.animation.add("holdend", [4 + event.strumID]);
	} else {
		note.loadGraphic(Paths.image('game/pixel/Notes'), true, 17, 17);
...
function onStrumCreation(event) {
	...
	strum.loadGraphic(Paths.image('game/pixel/Notes'), true, 17, 17);
